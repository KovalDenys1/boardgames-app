...
export default function ChessLobbiesPage() {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated') {
      ...
      if (!socket) {
        ...
        socket.on('connect', () => {
          console.log('✅ Socket connected for Chess lobby list')
          socket.emit('join-lobby-list')
        })

        socket.on('lobby-list-update', () => {
          console.log('📡 Chess lobby list update received')
          loadLobbies()
        })

        socket.on('disconnect', () => {
          console.log('❌ Socket disconnected from Chess lobby list')
        })
      }
      ...
      return () => {
        ...
        if (socket && socket.connected) {
          console.log('🔌 Disconnecting socket from Chess lobby list')
  ...
  const loadLobbies = async () => {
    try {
      const res = await fetch('/api/lobby?gameType=chess')
      const data = await res.json()
      setLobbies(data.lobbies || [])
    } catch (error) {
      console.error('Failed to load Chess lobbies:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-700 via-gray-800 to-black">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex items-center gap-2 text-white/80 text-sm">
          ...
          <span>›</span>
          <span className="text-white font-semibold">♟️ Chess Lobbies</span>
        </div>

        {/* Header */}
        <div className="mb-8 flex justify-between items-center">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2 drop-shadow-lg">♟️ Chess Lobbies</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl shadow-2xl p-8 text-white hover:shadow-3xl transition-all hover:scale-105 cursor-pointer border-4 border-white/20"
            ...
            <p className="text-white/90 mb-6 text-lg">Start your own Chess game and invite friends to join!</p>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl shadow-lg p-8 hover:shadow-xl transition-shadow border-2 border-white/20">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="px-8 py-3 bg-white text-gray-800 rounded-xl font-bold hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:scale-105 shadow-lg"
              >
                Join
              </button>
            </div>
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl shadow-lg p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-2xl font-bold text-white">Active Chess Lobbies</h2>
